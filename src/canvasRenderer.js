...
const {
  ...
  createFallbackImage,
  decodeHtmlEntities,
} = require("./utils"); // Import necessary utils
...
function drawLikeIcon(
  ...
  logger.info(
    `[drawLikeIcon] Received isLiked: ${isLiked} (type: ${typeof isLiked})`
  );
  ...
  ctx.bezierCurveTo(
    ...
    100 - pathOriginX,
    392 - pathOriginY
  ); // C 121 495 100 445 100 392 (Approximation, original SVG uses points)
  ctx.bezierCurveTo(
    100 - pathOriginX,
    339 - pathOriginY,
    121 - pathOriginX,
  ...
  if (isLiked === true) {
    ...
    logger.info(
      `[drawLikeIcon] Action: Stroking dimmed (isLiked is null/undefined)`
    );
      ...
      switch (type) {
        case "text":
          ...
          const displayText = maxWidth
            ? truncateText(ctx, text, maxWidth - padding * 2)
            : text;
...
async function createModernNowPlayingCanvas(config) {
  ...
  const {
    ...
    progressBarColor = "#ff0033", // Add default progressBarColor parameter
  ...
  try {
    ...
    if (buttonType === "like") {
      ...
      logger.info(
        `[createModernNowPlayingCanvas] Rendering 'like' button with isLiked: ${isLiked}, bgColor: ${likeBgColor}`
      );

      const finalLikedColor = likedColor || "#ff0033";
    ...
    if (albumArtUrl) {
      ...
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
          ...
          logger.warn(
            `Failed to load album art (Attempt ${attempt}/${MAX_RETRIES}) from ${albumArtUrl}: ${imgError.message}`
          );
          if (attempt === MAX_RETRIES) {
            logger.error(
              `Failed to load album art after ${MAX_RETRIES} attempts. Using fallback.`
            );
    ...
    if (showTimeInfo && duration > 0) {
      ...
      const finalTimeFontSize = Math.max(
        8,
        Math.min(24, parsedTimeFontSize || 10)
      );
    ...
    const adjustedAvailableTextWidth =
      availableTextWidth - (showTimeInfo ? timeTextWidth : 0);
    ...
    if (showTitle && decodedTrackName && availableTextWidth > 10) {
      ...
      let displayTrackName = truncateText(
        ctx,
        decodedTrackName,
        adjustedAvailableTextWidth
      );
    ...
    if (decodedArtistName && availableTextWidth > 10) {
      ...
      let displayArtistName = truncateText(
        ctx,
        decodedArtistName,
        adjustedAvailableTextWidth
      );
...
async function createYoutubeButtonDataUrl(
  ...
  try {
    ...
    let progressBarColor = "#ff0033"; // Default green color
    ...
    const accentColor = style.accentColor || "#ff0033";
    ...
    logger.error("Error creating YTMusic Desktop button Data URL:", error);
...
async function renderInterpolatedNowPlaying(serialNumber, key) {
  ...
  const {
    ...
    artistFontSize,
    timeFontSize,
    progressBarColor, // Extract progressBarColor directly from key.data
  } = currentKeyData.data;
  ...
  try {
    ...
    const title = decodeHtmlEntities(
      currentTrackDetails?.name || "Nothing Playing"
    );
    ...
    logger.error(
      `Error rendering interpolated now playing key ${keyId}: ${error.message}`
    );
...
async function initializeNowPlayingKey(serialNumber, key) {
  ...
  const initialProgressBarColor = key.data?.progressBarColor || "#ff0033";
  ...
  key.data = {
    ...
    timeFontSize:
      key.data?.timeFontSize !== undefined
        ? typeof key.data.timeFontSize === "string"
          ? parseInt(key.data.timeFontSize, 10)
...
function startOrRestartNowPlayingUpdates(serialNumber, key) {
  ...
  const apiFetchIntervalId = setInterval(async () => {
    ...
    if (!keyExists) {
      ...
      if (latestKeyData?.data?.interpolationIntervalId) {
        logger.info(
          `Clearing interpolation interval for inactive key ${keyId}.`
        );
  ...
  const interpolationIntervalId = setInterval(async () => {
    ...
    if (!keyExists) {
      ...
      if (
        latestKeyData?.data?.interpolationIntervalId === interpolationIntervalId
      ) {
...
async function updateNowPlayingKey(
  ...
  if (key.data?.progressBarColor) {
    logger.info(
      `[updateNowPlayingKey] Incoming key has progressBarColor: ${key.data.progressBarColor}`
    );
  }
  ...
  if (key.data?.progressBarColor && currentKeyData.data) {
    ...
    logger.info(
      `[updateNowPlayingKey] Updated progressBarColor: ${oldColor} → ${key.data.progressBarColor}`
    );
  }
  ...
  let needsAuth = false;
  try {
    playbackState = await ytmApi.getCurrentPlayback();
  } catch (error) {
    logger.error(
      `Error fetching playback state for ${keyId}: ${error.message}`
    );
    fetchError = error;
  }
  ...
  if (trackId !== previousTrackId) {
    ...
    if (
      ...
      try {
        const savedStatus = await ytmApi.checkTracksSaved([trackId]);
        if (savedStatus && savedStatus.length > 0) {
          currentPlaybackState.isLiked = savedStatus[0];
          likedStatusChanged = true;
          logger.info(
            `Track ${trackId} liked status: ${currentPlaybackState.isLiked}`
          );
        } else {
  ...
  if (!currentKeyData || !currentKeyData.data) {
    logger.error(
      `Key data for ${keyUid} disappeared during update processing.`
    );
    keyManager.cleanupKey(serialNumber, keyUid); // Clean up if data is missing
    return;
  }
  ...
  if (key.data?.progressBarColor) {
    logger.debug(
      `[updateNowPlayingKey] Re-ensuring progressBarColor: ${key.data.progressBarColor}`
    );
    currentKeyData.data.progressBarColor = key.data.progressBarColor;
    if (!currentKeyData.style) currentKeyData.style = {};
    currentKeyData.style.progressBarColor = key.data.progressBarColor;
  ...
  if (shouldStartTimers) {
    ...
    logger.debug(
      `[updateNowPlayingKey] Key ${keyId} - Calling immediate renderInterpolatedNowPlaying...`
    );
    await renderInterpolatedNowPlaying(serialNumber, currentKeyData);
    logger.debug(
      `[updateNowPlayingKey] Key ${keyId} - Immediate render call complete.`
    );
  }
  ...
  if (trackChanged || likedStatusChanged) {
    logger.debug(
      `Track or liked status change detected. Updating relevant like keys.`
    );
    Object.keys(keyManager.activeKeys).forEach((activeKeyId) => {
      ...
      if (likeKey && likeKey.cid === "com.energy.ytmdesktop_integration.like") {
